...
void run_printf(AST *ast) 
{
    ...
    if (expr_type == STR_TYPE)
    {
        ...
        {
            char string[strlen(str_buf)];
            char *aux;
            
            aux =  strtok(str_buf, "%");
            strcpy(string, str_buf);
            aux = strtok(NULL, "%");           
            char aux2[strlen(aux) - 1];
            for (int c = 1; aux[c] != '\0'; c++)
                aux2[c-1] = aux[c];
            aux2[strlen(aux) - 1] = '\0';
            // printf("%s\n", aux2);
            ...
            {
                int s2 = popi(); // String pointer
                clear_str_buf();
                escape_str(get_string(st, s2), str_buf);
                
                printf("%s", strcat(strcat(string, str_buf),aux2));
            }    
...
void rec_run_ast(AST *ast) {
    ...
    switch(get_kind(ast)) {
        ...
        //case NE_NODE:			run_ne(ast);            break;
        case MINUS_NODE:		run_minus(ast);         break;
        case OVER_NODE:			run_over(ast);          break;
        case MOD_NODE:			run_mod(ast);           break;
        case PLUS_NODE:			run_plus(ast);          break;
        //case AND_NODE:			run_and(ast);           break;
        //case OR_NODE:			run_or(ast);            break;
        //case POST_INC_NODE:     run_post_inc(ast);        break;
        //case PRE_INC_NODE:		run_pre_inc(ast);       break;
        //case POST_DEC_NODE:     run_post_dec(ast);        break;
        //case PRE_DEC_NODE:		run_pre_dec(ast);       break;
        //case REF_NODE:			run_ref(ast);           break;
        //case DEREF_NODE:		run_dref(ast);          break;
        //case UPLUS_NODE:		run_uplus(ast);         break;
        //case UMINUS_NODE:		run_uminus(ast);        break;
        //case LNEG_NODE:			run_lneg(ast);          break;
        ...
        case EXPR_LIST_NODE:	run_expr_list(ast);     break;
        ...
        //case INIT_LIST_NODE:	run_init_list(ast);     break;
