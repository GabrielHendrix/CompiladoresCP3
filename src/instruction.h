...
typedef enum {
    ...
    OROR,            // OROR ix, iy, iz     ; ix <- (bool) iy || (bool) iz
        // Logical AND
    ANDAND,          // ANDAND ix, iy, iz     ; ix <- (bool) iy && (bool) iz
        // Equality
    EQUi,            // EQUi ix, iy, iz     ; ix <- iy == iz ? 1 : 0
    EQUf,            // EQUf ix, fy, fz     ; ix <- fy == fz ? 1 : 0
    EQUs,            // EQUs ix, iy, iz     ; ix <- str_tab[iy] == str_tab[iz] ? 1 : 0
        // Inequality
    NEQUi,            // EQUi ix, iy, iz     ; ix <- iy != iz ? 1 : 0
    NEQUf,            // EQUf ix, fy, fz     ; ix <- fy != fz ? 1 : 0
    NEQUs,            // EQUs ix, iy, iz     ; ix <- str_tab[iy] != str_tab[iz] ? 1 : 0
        // Less than
