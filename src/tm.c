...
StepResult run_mod(Instr instr) {
    int d = is[instr.o3];
    if (d == 0) return DIV_ZERO;
    is[instr.o1] = is[instr.o2] % d;
    pc++;
...
StepResult run_andand(Instr instr) {
    is[instr.o1] = is[instr.o2] && is[instr.o3];
    pc++;
    return OKAY;
}
...
StepResult run_nequi(Instr instr) {
    is[instr.o1] = is[instr.o2] != is[instr.o3];
    pc++;
    return OKAY;
}

StepResult run_nequf(Instr instr) {
    is[instr.o1] = fs[instr.o2] != fs[instr.o3];
    pc++;
    return OKAY;
}

StepResult run_nequs(Instr instr) {
    char *l = get_string(st, is[instr.o2]);
    char *r = get_string(st, is[instr.o3]);
    is[instr.o1] = ( strcmp(l, r) != 0 );
    pc++;
...
StepResult step() {
    ...
    switch(instr.op) {
        ...
        case MODi:  return run_mod(instr);
        case WIDf: return run_widf(instr);
        case OROR: return run_oror(instr);
        case ANDAND: return run_andand(instr);
        case EQUi: return run_equi(instr);
        case EQUf: return run_equf(instr);
        case EQUs: return run_equs(instr);
        case NEQUi: return run_nequi(instr);
        case NEQUf: return run_nequf(instr);
        case NEQUs: return run_nequs(instr);
